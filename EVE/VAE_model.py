...
class VAE_model(nn.Module):
    ...
    def __init__(
        self, model_name, data, encoder_parameters, decoder_parameters, random_seed
    ):
        ...
        if decoder_parameters["bayesian_decoder"]:
            self.decoder = VAE_decoder.VAE_Bayesian_MLP_decoder(
                params=decoder_parameters
            )
        else:
            self.decoder = VAE_decoder.VAE_Standard_MLP_decoder(
                params=decoder_parameters
            )
    ...
    def KLD_diag_gaussians(self, mu, logvar, p_mu, p_logvar):
        ...
        KLD = (
            0.5 * (p_logvar - logvar)
            + 0.5
            * (torch.exp(logvar) + torch.pow(mu - p_mu, 2))
            / (torch.exp(p_logvar) + 1e-20)
            - 0.5
        )
    ...
    def KLD_global_parameters(self):
        ...
        for param_type in ["weight", "bias"]:
            KLD_decoder_params += self.KLD_diag_gaussians(
                self.decoder.state_dict(keep_vars=True)[
                    "last_hidden_layer_" + param_type + "_mean"
                ].flatten(),
                self.decoder.state_dict(keep_vars=True)[
                    "last_hidden_layer_" + param_type + "_log_var"
                ].flatten(),
                zero_tensor,
                zero_tensor,
            )
        ...
        if self.decoder.include_sparsity:
            ...
            self.logit_scale_mu = (
                2.0**0.5
                * self.logit_scale_sigma
                * erfinv(2.0 * self.logit_sparsity_p - 1.0)
            )

            sparsity_mu = torch.tensor(self.logit_scale_mu).to(self.device)
            sparsity_log_var = torch.log(torch.tensor(self.logit_scale_sigma**2)).to(
                self.device
            )
            KLD_decoder_params += self.KLD_diag_gaussians(
                self.decoder.state_dict(keep_vars=True)[
                    "sparsity_weight_mean"
                ].flatten(),
                self.decoder.state_dict(keep_vars=True)[
                    "sparsity_weight_log_var"
                ].flatten(),
                sparsity_mu,
                sparsity_log_var,
            )
        ...
        if self.decoder.convolve_output:
            for param_type in ["weight"]:
                KLD_decoder_params += self.KLD_diag_gaussians(
                    self.decoder.state_dict(keep_vars=True)[
                        "output_convolution_mean." + param_type
                    ].flatten(),
                    self.decoder.state_dict(keep_vars=True)[
                        "output_convolution_log_var." + param_type
                    ].flatten(),
                    zero_tensor,
                    zero_tensor,
                )
        ...
        if self.decoder.include_temperature_scaler:
            KLD_decoder_params += self.KLD_diag_gaussians(
                self.decoder.state_dict(keep_vars=True)[
                    "temperature_scaler_mean"
                ].flatten(),
                self.decoder.state_dict(keep_vars=True)[
                    "temperature_scaler_log_var"
                ].flatten(),
                zero_tensor,
                zero_tensor,
            )
    ...
    def loss_function(
        ...
        """
        BCE = (
            F.binary_cross_entropy_with_logits(x_recon_log, x, reduction="sum")
            / x.shape[0]
        )
        KLD_latent = (
        ...
        neg_ELBO = BCE + warm_up_scale * (
            ...
            + kl_global_params_scale * KLD_decoder_params_normalized
        )
    ...
    def all_likelihood_components(self, x):
        ...
        BCE_batch_tensor = torch.sum(
            F.binary_cross_entropy_with_logits(recon_x_log, x, reduction="none"), dim=1
        )
        KLD_batch_tensor = -0.5 * torch.sum(
    ...
    def train_model(self, data, training_parameters):
        ...
        if training_parameters["log_training_info"]:
            filename = (
                training_parameters["training_logs_location"]
                + os.sep
                + self.model_name
                + "_losses.csv"
            )
            with open(filename, "a") as logs:
                logs.write(
                    "Number of sequences in alignment file:\t"
                    + str(data.num_sequences)
                    + "\n"
                )
        ...
        for training_step in tqdm.tqdm(
            range(1, training_parameters["num_training_steps"] + 1),
            desc="Training model",
        ):

            batch_index = np.random.choice(
                batch_order, training_parameters["batch_size"], p=seq_sample_probs
            ).tolist()
            ...
            neg_ELBO, BCE, KLD_latent, KLD_decoder_params_normalized = (
                self.loss_function(
                    recon_x_log,
                    x,
                    mu,
            ...
            if training_step % training_parameters["save_model_params_freq"] == 0:
                self.save(
                    model_checkpoint=training_parameters["model_checkpoint_location"]
                    + os.sep
                    + self.model_name
                    + "_step_"
                    + str(training_step),
            ...
            if (
                training_parameters["use_validation_set"]
                and training_step % training_parameters["validation_freq"] == 0
            ):
                x_val = torch.tensor(x_val, dtype=self.dtype).to(self.device)
                val_neg_ELBO, val_BCE, val_KLD_latent, val_KLD_global_parameters = (
                    self.test_model(
                        x_val, weights_val, training_parameters["batch_size"]
                    )
                )
                ...
                if val_neg_ELBO < best_val_loss:
                    ...
                    self.save(
                        model_checkpoint=training_parameters[
    ...
    def test_model(self, x_val, weights_val, batch_size):
        ...
        with torch.no_grad():
            ...
            val_batch_index = np.random.choice(
                val_batch_order, batch_size, p=val_seq_sample_probs
            ).tolist()
            ...
            neg_ELBO, BCE, KLD_latent, KLD_global_parameters = self.loss_function(
                recon_x_log,
                x,
                mu,
                log_var,
    ...
    def compute_evol_indices(
        ...
        list_valid_mutated_sequences["wt"] = (
            msa_data.focus_seq_trimmed
        ...
        for mutation in list_mutations["mutations"]:
            ...
            for mut in individual_substitutions:
                ...
                if (
                    pos not in msa_data.uniprot_focus_col_to_wt_aa_dict
                    ...
                    wt_aa, pos, idx_focus = (
                        msa_data.mutant_to_letter_pos_idx_focus_list[mut]
                    )
                    mutated_sequence[idx_focus] = (
                        mut_aa  # perform the corresponding AA substitution
                    )
        ...
        mutated_sequences_one_hot = np.zeros(
            (
                len(list_valid_mutations),
                len(msa_data.focus_cols),
                len(msa_data.alphabet),
            )
        )
    ...
    def get_latent_representation(self, data, device):
        """
        For every sequence in data, an MSA_processing object,
        get the latent representation from the trained VAE model

        Returns a dictionary mapping sequence name to latent representation
        """
        latent_reps = {}
        self.eval()
        with torch.no_grad():
            for _, seq_name in enumerate(data.seq_name_to_sequence.keys()):
                sequence = data.seq_name_to_sequence[seq_name]
                encoded_seq = np.zeros(len(sequence), len(data.alphabet))
