...
class VAE_Bayesian_MLP_decoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        for layer_index in range(len(self.hidden_layers_sizes)):
            if layer_index == 0:
                ...
                nn.init.constant_(
                    self.hidden_layers_mean[str(layer_index)].bias, self.mu_bias_init
                )
                nn.init.constant_(
        ...
        if self.include_sparsity:
            self.sparsity_weight_mean = nn.Parameter(
                torch.zeros(
                    int(self.hidden_layers_sizes[-1] / self.num_tiles_sparsity),
                    self.seq_len,
                )
            )
        ...
        if self.include_temperature_scaler:
            ...
            self.temperature_scaler_log_var = nn.Parameter(
                torch.ones(1) * self.logvar_init
            )
    ...
    def forward(self, z):
        ...
        b_out = self.sampler(
            self.last_hidden_layer_bias_mean, self.last_hidden_layer_bias_log_var
        )
        ...
        if self.convolve_output:
            ...
            W_out = torch.mm(
                W_out.view(
                    self.seq_len * self.hidden_layers_sizes[-1], self.channel_size
                ),
        ...
        if self.include_sparsity:
            sparsity_weights = self.sampler(
                self.sparsity_weight_mean, self.sparsity_weight_log_var
            )
            ...
            W_out = (
                W_out.view(
                    self.hidden_layers_sizes[-1], self.seq_len, self.alphabet_size
                )
                * sparsity_tiled
            )

        W_out = W_out.view(
            self.seq_len * self.alphabet_size, self.hidden_layers_sizes[-1]
        )
        ...
        x_recon_log = F.log_softmax(
            x, dim=-1
        )  # of shape (batch_size, seq_len, alphabet)
...
class VAE_Standard_MLP_decoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        for layer_index in range(len(self.hidden_layers_sizes)):
            if layer_index == 0:
                ...
                nn.init.constant_(
                    self.hidden_layers[str(layer_index)].bias, self.mu_bias_init
                )
            else:
                self.hidden_layers[str(layer_index)] = nn.Linear(
                    self.hidden_layers_sizes[layer_index - 1],
        ...
        if self.include_sparsity:
            self.sparsity_weight = nn.Parameter(
                torch.randn(
                    int(self.hidden_layers_sizes[-1] / self.num_tiles_sparsity),
                    self.seq_len,
                )
    ...
    def forward(self, z):
        ...
        x = self.last_hidden_nonlinearity(
            self.hidden_layers[str(len(self.hidden_layers_sizes) - 1)](x)
        )  # of size (batch_size,H)
        if self.dropout_proba > 0.0:
            x = self.dropout_layer(x)
        ...
        if self.convolve_output:
            W_out = torch.mm(
                ...
                self.output_convolution.weight.view(
                    self.channel_size, self.alphabet_size
                ),
            )

        if self.include_sparsity:
            sparsity_tiled = self.sparsity_weight.repeat(
                self.num_tiles_sparsity, 1
            )  # of size (H,seq_len)
            sparsity_tiled = nn.Sigmoid()(sparsity_tiled).unsqueeze(
                2
            )  # of size (H,seq_len,1)
            W_out = (
                W_out.view(
