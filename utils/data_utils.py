...
class MSA_processing:
    ...
    def gen_alignment(self):
        ...
        if self.preprocess_MSA:
            msa_df = pd.DataFrame.from_dict(
                self.seq_name_to_sequence, orient="index", columns=["sequence"]
            )
            # Data clean up
            msa_df.sequence = msa_df.sequence.apply(
                lambda x: x.replace(".", "-")
            ).apply(lambda x: "".join([aa.upper() for aa in x]))
            # Remove columns that would be gaps in the wild type
            non_gap_wt_cols = [aa != "-" for aa in msa_df.sequence[self.focus_seq_name]]
            msa_df["sequence"] = msa_df["sequence"].apply(
                lambda x: "".join(
                    [aa for aa, non_gap_ind in zip(x, non_gap_wt_cols) if non_gap_ind]
                )
            )
            assert (
                0.0 <= self.threshold_sequence_frac_gaps <= 1.0
            ), "Invalid fragment filtering parameter"
            assert (
                0.0 <= self.threshold_focus_cols_frac_gaps <= 1.0
            ), "Invalid focus position filtering parameter"
            msa_array = np.array([list(seq) for seq in msa_df.sequence])
            gaps_array = np.array(
                list(map(lambda seq: [aa == "-" for aa in seq], msa_array))
            )
            ...
            print(
                "Proportion of sequences dropped due to fraction of gaps: "
                + str(
                    round(
                        float(1 - seq_below_threshold.sum() / seq_below_threshold.shape)
                        * 100,
                        2,
                    )
                )
            ...
            index_cols_below_threshold = (
                columns_gaps_frac <= self.threshold_focus_cols_frac_gaps
            )
            print(
                "Proportion of non-focus columns removed: "
                + str(
                    round(
                        float(
                            1
                            - index_cols_below_threshold.sum()
                            / index_cols_below_threshold.shape
                        )
                        * 100,
                        2,
                    )
            ...
            msa_df["sequence"] = msa_df["sequence"].apply(
                lambda x: "".join(
                    [
                        aa.upper() if upper_case_ind else aa.lower()
                        for aa, upper_case_ind in zip(x, index_cols_below_threshold)
                    ]
                )
            )
            ...
            for seq_idx in range(len(msa_df["sequence"])):
                self.seq_name_to_sequence[msa_df.index[seq_idx]] = msa_df.sequence[
                    seq_idx
                ]

        self.focus_seq = self.seq_name_to_sequence[self.focus_seq_name]
        self.focus_cols = [
            ix for ix, s in enumerate(self.focus_seq) if s == s.upper() and s != "-"
        ]
        ...
        for seq_name, sequence in self.seq_name_to_sequence.items():
            ...
            self.seq_name_to_sequence[seq_name] = [
                sequence[ix].upper() for ix in self.focus_cols
            ]
        ...
        self.one_hot_encoding = np.zeros(
            (
                len(self.seq_name_to_sequence.keys()),
                len(self.focus_cols),
                len(self.alphabet),
            )
        )
